...
- **Dossier cible :** `images/medocs/`
- **Nom de fichier :** `{quiz}-{slug}.webp` ou `.jpg` (ex. `soin-peau-hydrance-riche.webp`) — slug donné dans la colonne « Slug photo » ci-dessous
- **Format :** `.webp` de préférence (packshot fournisseur), `.jpg` accepté. Noms ASCII uniquement — pas d'accents, pas d'espaces, pas de `|`
- **Cadrage :** produit centré, fond clair ou transparent, pas d'ombre dans l'image (l'ombre portée elliptique est ajoutée en CSS côté site)
- **État colonne :** ☐ à faire · ✅ fait (mettre à jour cette page au fur et à mesure)

> Workflow : packshot → renommer en `{slug}.webp` → déposer dans `images/medocs/` → ajouter `imageFile: '{slug}.webp'` après la ligne `emoji:` du produit dans le JS du quiz → cocher ✅ ici.
>
> ⚠️ Insérer `imageFile` **par clé produit**, jamais par numéro de ligne. Un `sed '{N}a\...'` sur ce fichier a déjà injecté des lignes au milieu de tableaux et cassé les deux quiz (réparé le 2026-08-09).
...
**Les 4 quiz sont branchés.** Chacun affiche `<img src="../images/medocs/{imageFile}">` quand le produit porte un champ `imageFile`, et retombe sur l'emoji sinon. Ajouter une photo = déposer le fichier dans `images/medocs/` + ajouter la ligne `imageFile` au produit. Rien d'autre à toucher.
...
- **Naturels et compléments : pastille ronde de 52 px** (`.protocol-emoji`), pas une grande vignette comme les deux autres quiz. Une règle CSS `.protocol-emoji img` (`object-fit: contain`, `border-radius: 50%`) a été ajoutée dans les deux fichiers. Cadrer les packshots serré, le produit sera vu petit.
- **Où insérer `imageFile` :** juste après la ligne `emoji:` du produit. Dans `quiz-produits-naturels.html` les produits sont des objets anonymes dans `programmes[x].products[]` — repérer le bloc par son `name:`, pas par une clé.
- Garder le `alt` descriptif (nom produit + marque) — déjà généré à partir de `p.name`.
- **Extensions mélangées** dans `images/medocs/` : `.webp`, `.jpg`, `.jpeg`, `.png`. Le champ `imageFile` porte l'extension, donc n'importe laquelle passe. **Pas d'AVIF** — le format exclut Safari < 16 ; `complement-curcuma-bio` a été reconverti en WebP le 2026-08-09.
