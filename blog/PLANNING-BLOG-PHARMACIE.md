...
| 32 | Lun 31 août 2026 | Rentrée: comment remettre votre ado dans le rythme | Long éditorial | PiLeJe (magnésium) | ✏️ **RÉDIGÉ 25 août 2026** (dossier `33-rentree-rythme-ado/`, `.md` + `.html` + GMB #36 rédigés. Perplexity toujours hors service: recherche via WebSearch, chaque source vérifiée une à une (règlement UE 432/2012, ANSES caféine, EnCLASS 2022, consensus vitamine D SFP/AFPA 2022, HAS 2024 fer, VIDAL ferritine, Ameli bilan prévention 18-25 ans). Angle validé par Dany: répondre « rarement » à la demande de compléments et déplacer le conseil sur le rythme circadien, le petit-déjeuner et les deux dosages qui comptent. Shortlink da.gd/hGSw7z (is.gd en panne, « database insert failed »). Double-lien nav posé avec #31. Reste à faire: image hero + OG (`blog-36-` / `og-36-rentree-rythme-ado.jpg`), `blog.html` + `sitemap.xml` + `feed.xml` + `llms.txt`, publication GMB). |
...
**Prochain article à publier**: #32 (Rentrée: remettre son ado dans le rythme) rédigé le 25 août 2026 pour la date cible du 31 août. Reste: image hero/OG, mise en ligne (`blog.html`, `sitemap.xml`, `feed.xml`, `llms.txt`), publication GMB #36. Créneau suivant: lundi 14 ou 21 septembre 2026, Poux de rentrée (`34-poux-rentree/`).
**Dernière mise à jour planning**: 25 août 2026 (#32 Rentrée/concentration ado rédigé ; #31 Coup de chaleur passé en PUBLIÉ après vérification de `blog.html`, `sitemap.xml`, `feed.xml`, `llms.txt` et des images ; Poux repoussé à mi-septembre avec la raison écrite ci-dessus).
